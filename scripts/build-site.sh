...
<head>
  ...
  <title>aver - GitHub Actions Version Checker</title>
  <style>
    body {
...
<body>
  <nav>
    ...
    <a href="https://github.com/llimllib/aver">${GITHUB_ICON}GitHub</a>
  </nav>
HTMLEOF
...
<body>
  ...
  <p>AI coding session transcripts for the aver project.</p>
  <ul>
HTMLEOF
...
for f in transcripts/*.html; do
    if [ -f "$f" ]; then
        ...
        echo "    <li><a href=\"$basename\">$name</a><a class=\"pr-link\" href=\"https://github.com/llimllib/aver/pull/$pr_number\">${GITHUB_ICON}PR #$pr_number</a></li>" >> _site/transcripts/index.html
    fi
done
